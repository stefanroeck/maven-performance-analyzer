...
export const fillsForAllColors: SvgDefsAndFill<
  ComputedNode<TreeMapNode>
>["fill"] = treeMapColors.flatMap((color, idx) => {
  return [
    {
      match: (node): boolean =>
        node.data.moduleId !== undefined &&
        colorIdx(node.data.moduleId) === idx &&
        node.id === "mainSrc",
      id: `${idx}_mainSrc`,
    },
    {
