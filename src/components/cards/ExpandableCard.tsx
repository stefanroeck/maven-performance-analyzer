...
export const ExpandableCard: FC<Props> = ({ children, title, subheader, expanded: initiallyExpanded = false, onExpanded = () => { } }) => {
    ...
    return (
        <Card sx={{ margin: "20px 0" }}>
            <CardHeader title={title} subheader={subheader} action={
                <IconButton onClick={handleOnExpandIconClick}>{expanded ? <ExpandLessIcon /> : <ExpandMoreIcon />}</IconButton>
            } />
